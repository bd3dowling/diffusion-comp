...
def get_optimizer(config):
    ...
    if config.optim.optimizer == "Adam":
        ...
        raise NotImplementedError(f"Optimiser {config.optim.optimizer} not supported yet!")
...
def train(sampling_shape, config, dataset, workdir=None, use_wandb=False):
    ...
    if jax.process_index() == 0:
        print(f"num_devices={num_devices}, pmap={config.training.pmap}")
    ...
    params = model.init(model_rng, jnp.zeros(sampling_shape), jnp.ones((sampling_shape[0],)))
    ...
    if workdir is not None:
        ...
        manager_options = orbax.checkpoint.CheckpointManagerOptions(create=True, max_to_keep=np.inf)
    ...
    for i_epoch in trange(1, num_epochs + 1, unit="epochs"):
        ...
        if i_epoch != 0 and (num_epochs < config.training.batch_size):
            print(f"Epoch took {current_time - prev_time:.1f} seconds")
            prev_time = time.time()
            ...
            for i_batch, batch in enumerate(tepoch):
                ...
                if jax.process_index() == 0:
                    ...
                    if (
                        ...
                        logging.info(f"step {step:d}, training_loss {loss_train:.2e}")
                ...
                if step % config.training.eval_freq == 0:
                    ...
                    if jax.process_index() == 0 and use_wandb:
                        logging.info(f"batch: {step:d}, eval_loss: {loss_eval:.5e}")
                    ...
                    if config.training.snapshot_sampling:
                        ...
                        if workdir:
                            this_sample_dir = os.path.join(
                                sample_dir, f"iter_{step}_host_{jax.process_index()}"
                            )
        ...
        if jax.process_index() == 0 and i_epoch % config.training.log_epoch_freq == 0:
            ...
            if use_wandb:
                logging.info(f"step {int(step):d}, mean_loss {float(mean_loss[0]):.2e}")
