...
from diffusionlib.util.misc import batch_mul, get_linear_beta_function, get_sigma_function
...
class VP:
    ...
    def __init__(self, beta=None, log_mean_coeff=None):
        if beta is None:
            self.beta, self.log_mean_coeff = get_linear_beta_function(beta_min=0.1, beta_max=20.0)
        else:
            self.beta = beta
