...
from diffusionlib.model.layers import score, up_or_down_sampling

conv1x1 = score.ddpm_conv1x1
conv3x3 = score.ddpm_conv3x3
NIN = score.NIN
default_init = score.default_init
...
class CondRefineBlock(nn.Module):
    ...
    def __call__(self, xs, y):
        rcu_block = functools.partial(
            CondRCUBlock, n_blocks=2, n_stages=2, act=self.act, normalizer=self.normalizer
        )
        rcu_block_output = functools.partial(
            CondRCUBlock,
            features=self.features,
...
###########################################################################
# Layers for definining NCSN++
#  originally in `models.layerspp` from `score_sde` repo
###########################################################################
