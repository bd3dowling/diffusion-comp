...
def conv_downsample_2d(x, w, k=None, factor=2, gain=1, data_format="NHWC"):
    ...
    p = (k.shape[0] - factor) + (convW - 1)
    s = [factor, factor]
    x = _simple_upfirdn_2d(x, k, pad0=(p + 1) // 2, pad1=p // 2, data_format=data_format)
...
def upfirdn_2d(x, k, upx, upy, downx, downy, padx0, padx1, pady0, pady1):
    ...
    x = jnp.pad(x, [[0, 0], [max(pady0, 0), max(pady1, 0)], [max(padx0, 0), max(padx1, 0)], [0, 0]])
    x = x[
        :,
    ...
    x = jax.lax.conv_general_dilated(
        x, w, window_strides=(1, 1), padding="VALID", dimension_numbers=("NCHW", "HWIO", "NCHW")
    )
...
def _simple_upfirdn_2d(x, k, up=1, down=1, pad0=0, pad1=0, data_format="NCHW"):
    ...
    y = upfirdn_2d(
        y, k, upx=up, upy=up, downx=down, downy=down, padx0=pad0, padx1=pad1, pady0=pad0, pady1=pad1
    )
    if data_format == "NCHW":
...
def upsample_2d(x, k=None, factor=2, gain=1, data_format="NHWC"):
    ...
    p = k.shape[0] - factor
    return _simple_upfirdn_2d(
        x, k, up=factor, pad0=(p + 1) // 2 + factor - 1, pad1=p // 2, data_format=data_format
    )
