...
def shake_drop_train(x, mask_prob, alpha_min, alpha_max, beta_min, beta_max, rng=None):
    ...
    return x * rand_backward + jax.lax.stop_gradient(x * rand_forward - x * rand_backward)
...
class WideResnet(nn.Module):
    ...
    def __call__(self, x, sigmas, train=True):
        ...
        x = WideResnetGroup(
            self.blocks_per_group, 16 * self.channel_multiplier, activate_before_residual=True
        )(x, temb, train)
        x = WideResnetGroup(self.blocks_per_group, 32 * self.channel_multiplier, (2, 2))(
            x, temb, train
        )
        x = WideResnetGroup(self.blocks_per_group, 64 * self.channel_multiplier, (2, 2))(
            x, temb, train
        )
